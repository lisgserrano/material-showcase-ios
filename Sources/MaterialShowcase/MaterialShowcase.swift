...
@objc public protocol MaterialShowcaseDelegate: class {
    ...
    @objc optional func dismissAllShowCases()
}
...
public class MaterialShowcase: UIView {
    ...
    @objc public enum TargetHolderStyle: Int {
          case stroke
          case fill
          case strokeRect
      }
    ...
    @objc public var targetHolderStyle:  TargetHolderStyle = .fill
    ...
    public var shouldEnableNextButton: Bool = false
    public var shouldEnableSkipButton: Bool = false
    @objc public var buttonsAccentColor: UIColor!
    @objc public var nextButtonText: String!
    @objc public var skipButtonText: String!
...
extension MaterialShowcase {
    ...
    func setDefaultProperties() {
        ...
        buttonsAccentColor = MaterialShowcaseInstructionView.PRIMARY_TEXT_COLOR
        nextButtonText = MaterialShowcaseInstructionView.NEXT_BTN_DEFAULT_TEXT
        skipButtonText = MaterialShowcaseInstructionView.SKIP_BTN_DEFAULT_TEXT
    ...
    func initViews() {
        ...
        if targetView.frame.width > 200 {
            self.targetHolderStyle = .strokeRect
        }
       ...
       // subviews.forEach({$0.isUserInteractionEnabled = false})
        ...
        if shouldEnableSkipButton {
            instructionView.skipButton.addGestureRecognizer(tapGestureRecoganizer())
            instructionView.skipButton.isUserInteractionEnabled = true
        }
        
        if shouldEnableNextButton {
            instructionView.nextButton.addGestureRecognizer(tapGestureRecoganizer())
            instructionView.nextButton.isUserInteractionEnabled = true
         }
        ...
        if isTapRecognizerForTargetView {
            ...
            if let bg = self.viewWithTag(2) {
                bg.addGestureRecognizer(tapGestureRecoganizer())
    ...
    private func addBackground() {
        ...
        case .bezelRect:
            ...
            let width =  (targetRippleView.frame.minX - 10 <= UIScreen.main.bounds.minX || targetRippleView.frame.maxX + 10 >= UIScreen.main.bounds.maxX) ? UIScreen.main.bounds.width : UIScreen.main.bounds.width - 20
            let height = targetRippleView.frame.minY < UIScreen.main.bounds.minY ? 400 : targetRippleView.frame.height + 300
            let yPosition = (targetRippleView.center.y > self.center.y) ? targetRippleView.frame.minY - 280 : targetRippleView.frame.minY - 20 > UIScreen.main.bounds.minY ? targetRippleView.frame.minY - 20 : 0
            backgroundView = UIView(frame: CGRect(x: 0, y: yPosition, width: width, height: height))
            var center = backgroundView.center
            center.x = self.center.x
            backgroundView.center = center
            backgroundView.asBezelRect()
        }
        ...
        if self.backgroundViewType != .bezelRect {
    ...
    private func addOpaqueBackground() {
        let oBg = UIView(frame: CGRect(x: 0, y: 0, width: UIScreen.main.bounds.width, height: UIScreen.main.bounds.height))
        oBg.backgroundColor = UIColor.black.withAlphaComponent(0.3)
        oBg.tag = 2
        insertSubview(oBg, belowSubview: backgroundView)
    }
    ...
    private func addTargetHolder(at center: CGPoint) {
        ...
        targetHolderView = targetHolderStyle == .strokeRect ? UIView(frame: CGRect(x: 0, y: 0, width: targetView.frame.width + 40 ,height: targetView.frame.height + 40)) : UIView(frame: CGRect(x: 0, y: 0, width: targetHolderRadius * 2,height: targetHolderRadius * 2))
        targetHolderView.center = center
        if targetHolderStyle == .fill {
            targetHolderView.backgroundColor = targetHolderColor
        } else {
            targetHolderView.layer.borderWidth = 5
            targetHolderView.layer.borderColor = targetHolderColor.cgColor
        }
        if targetHolderStyle == .strokeRect {
            targetHolderView.roundBorder()
        } else {
    ...
    private func addInstructionView(at center: CGPoint) {
        ...
        instructionView.buttonsAccentColor = buttonsAccentColor
        instructionView.nextButtonText = nextButtonText
        instructionView.skiptButtonText = skipButtonText
        ...
        instructionView.nextActionEnabled = shouldEnableNextButton
        instructionView.skipActionEnabled = shouldEnableSkipButton
        instructionView.targetPosition = getTargetPosition(target: targetView, container: containerView)
        ...
        if UIDevice.current.userInterfaceIdiom == .pad {
            ...
            if getTargetPosition(target: targetView, container: containerView) == .above {
                yPosition = targetHolderView.frame.maxY + 50 > 100 ? targetHolderView.frame.maxY + 50 : center.y + TARGET_PADDING + (targetView.bounds.height / 2 > targetHolderRadius ? targetView.bounds.height / 2 : targetHolderRadius)
            } else {
                let bgYPosition = (targetRippleView.center.y > self.center.y) ? targetRippleView.frame.minY - 280 : targetRippleView.frame.minY - 20 > UIScreen.main.bounds.minY ? targetRippleView.frame.minY - 20 : 0
                yPosition = targetHolderView.frame.minX < center.x ? bgYPosition + 50 : center.y - TEXT_CENTER_OFFSET - LABEL_DEFAULT_HEIGHT * 2 - 100
            }
    ...
    /// Handles user's tap
    private func tapGestureRecoganizer(target: Any? = nil) -> UIGestureRecognizer {
        let targetView = target != nil ?  target : self
        let tapGesture = UITapGestureRecognizer(target: targetView, action: #selector(MaterialShowcase.tapGestureSelector))
    ...
    @objc private func tapGestureSelector(tapGesture:UITapGestureRecognizer) {
        if tapGesture.view === instructionView.skipButton {
            self.delegate?.dismissAllShowCases?()
